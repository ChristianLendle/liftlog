...
// Supabase mock injected before page scripts so the auth gate auto-dismisses.
function sbMock() {
  let _sb;
  Object.defineProperty(window, 'supabase', {
    ...
    get() {
      ...
      return {
        createClient: () => ({
          auth: {
            getUser: async () => ({ data: { user: { id: 'tid', email: 't@t.com' } }, error: null }),
            onAuthStateChange: () => ({ data: { subscription: { unsubscribe: () => {} } } }),
            signOut: async () => ({ error: null }),
            updateUser: async () => ({ data: { user: {} }, error: null }),
            resetPasswordForEmail: async () => ({ data: {}, error: null }),
            signInWithPassword: async () => ({ data: { user: { id: 'tid' }, session: {} }, error: null }),
          },
          from: chain,
